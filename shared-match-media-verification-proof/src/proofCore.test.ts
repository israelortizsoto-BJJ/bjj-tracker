...
test("deterministic proof identity binds immutable proof inputs", async () => {
...
test("proof request admits only exact isolated benchmark objects", () => {
  assert.equal(isProofRequest(request), true);
  assert.equal(isProofRequest({ ...request, objectKey: "production/object.mp4" }), false);
  assert.equal(isProofRequest({ ...request, expectedBytes: 1024 }), false);
...
test("bounded MIME detection accepts only the deterministic MP4 prefix", () => {
...
test("seeding validators admit only proof object and upload identities", () => {
  ...
  assert.equal(benchmarkExpectedBytes("benchmarks/20gib-v1.mp4"), 20 * 1024 * 1024 * 1024);
